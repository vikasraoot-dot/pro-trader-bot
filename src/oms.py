...
class OMS:
    ...
    def _bar_interval_str(self) -> str:
        # yfinance intervals: '15m' OK, '60m' OK
        return self.cfg.general.bar_timeframe

    def _fetch(self, sym: str, start: str, end: str, interval: str) -> pd.DataFrame:
        return download_ohlc(sym, start, end, interval)

    def trade_cycle(self):
        # decide universe dynamically: read last 60d to compute liquidity
        today = pd.Timestamp.utcnow().strftime("%Y-%m-%d")
        start = (pd.Timestamp.utcnow() - pd.Timedelta(days=90)).strftime("%Y-%m-%d")

        # In practice, you'd load from a static tickers list; here a small example:
        ...
        for sym in long_symbols:
            ...
            if plan.qty <= 0:
                continue
            coid = gen_coid(sym, pd.Timestamp.utcnow().strftime("%Y%m%d%H%M"), f"{sym}|{last_price}|{plan.qty}|{plan.stop_price}|{plan.take_profit}")
            ...
            self.logger.info({"event":"order_submitted","symbol":sym,"qty":plan.qty,"entry":last_price,"tp":plan.take_profit,"sl":plan.stop_price})
